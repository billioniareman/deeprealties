...
def encode_mongodb_uri(uri: str) -> str:
    ...
    try:
        ...
        if uri.startswith("mongodb+srv://"):
            # Extract the connection string parts
            if "@" in uri:
        ...
        # Handle standard mongodb:// URIs
            ...
            if parsed.username or parsed.password:
                ...
                netloc = f"{username}:{password}@{parsed.hostname}" if password else f"{username}@{parsed.hostname}"
                if parsed.port:
                    netloc = f"{netloc.split('@')[0]}@{parsed.hostname}:{parsed.port}" if '@' in netloc else f"{parsed.hostname}:{parsed.port}"
